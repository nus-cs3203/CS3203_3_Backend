...
auto BaseApiHandler::count_documents(
    ...
    std::shared_ptr<DatabaseManager> db_manager, 
    const std::string& collection_name, 
    std::function<std::tuple<bsoncxx::document::value, mongocxx::options::count>(const crow::request&)> process_request_func,
    ...
    try {
        ...
        return BaseApiStrategyUtils::make_success_response(200, response_data, "Server processed count_documents request successfully.");
    }
    catch (const std::exception& e) {
        return BaseApiStrategyUtils::make_error_response(500, std::string("Server error: ") + e.what());
    }
