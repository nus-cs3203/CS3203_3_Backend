...
EnvManager::EnvManager(const std::string& filename) { _load_env_file(filename); }

auto EnvManager::read_env(const std::string& key, const std::string& default_value) const
    -> std::string {
    const char* env_var = std::getenv(key.c_str());
    if (!env_var) {
        std::cout << "[EnvManager] Environment variable '" << key
