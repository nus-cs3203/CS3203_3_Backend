...
TEST(AnalyticsApiHandlerTest, GetOneByName) {
    ...
    auto doc = make_document(kvp("name", "Housing"),
                             kvp("summary", "Test summary"));
...
TEST(AnalyticsApiHandlerTest, GetComplaintsStatistics) {
    ...
    db_ptr->insert_one(collection, make_document(kvp("category", "Housing"), kvp("sentiment", -0.8)).view());
    db_ptr->insert_one(collection, make_document(kvp("category", "Housing"), kvp("sentiment", -0.82)).view());
...
TEST(AnalyticsApiHandlerTest, GetComplaintsStatisticsOverTime) {
    ...
    db_ptr->insert_one(collection, 
        make_document(kvp("category", "Housing"),
                      kvp("sentiment", -0.5),
                      kvp("date", bsoncxx::types::b_date{std::chrono::system_clock::from_time_t(raw_time)})).view());

    // Build a request that includes _from_date and _to_date.
    crow::request req;
    req.body = "{\"filter\": {\"_from_date\": \"01-01-2023 00:00:00\", \"_to_date\": \"02-01-2023 00:00:00\"}}";
...
TEST(AnalyticsApiHandlerTest, GetComplaintsStatisticsGrouped) {
    ...
    db_ptr->insert_one(collection, make_document(kvp("category", "Housing"), kvp("sentiment", -0.5)).view());
    db_ptr->insert_one(collection, make_document(kvp("category", "Housing"), kvp("sentiment", -0.3)).view());
    db_ptr->insert_one(collection, make_document(kvp("category", "Transportation"), kvp("sentiment", 0.7)).view());
...
TEST(AnalyticsApiHandlerTest, GetComplaintsStatisticsGroupedOverTime) {
    ...
    crow::request req;
    req.body = "{\"group_by_field\": \"category\", \"filter\": {\"_from_date\": \"01-01-2023 00:00:00\", \"_to_date\": \"02-01-2023 00:00:00\"}}";
...
TEST(AnalyticsApiHandlerTest, GetComplaintsStatisticsGroupedBySentimentValue) {
    ...
    crow::request req;
    req.body = "{\"bucket_size\": 0.5, \"filter\": {\"_from_date\": \"01-01-2010 00:00:00\", \"_to_date\": \"31-12-2010 23:59:59\"}}";
    auto response = handler.get_complaints_statistics_grouped_by_sentiment_value(req, db_ptr, collection);
    EXPECT_EQ(response.code, 200);
    EXPECT_NE(response.body.find("Server processed aggregate request successfully"), std::string::npos);
